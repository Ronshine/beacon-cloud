...
@Slf4j
public class BlackGlobalFilter implements StrategyFilter {
    @Autowired
    ...
    public void strategy(StandardSubmit submit) {
        log.info("【策略模块-平台级黑名单校验】 校验ing ~ ~");

        //1、获取用户电话号码
        String mobile = submit.getMobile();

        //2、查询平台级用户黑名单
        String value = cacheClient.getString(SmsConstant.BLACK + mobile);

        //3、判断该号码是否是平台级黑名单
        if (TRUE.equals(value)){
            //发送消息和推送报告和抛出异常
            log.info("【策略模块-平台级黑名单校验】该号码为平台级黑名单无法发送消息 mobile = {}",mobile);
            submit.setErrorMsg("该用户电话号码为平台级黑名单无法发送消息，请到线下营业厅办理解除业务 mobile = " + mobile);
            sendMsgUtil.sendLog(submit);
            sendMsgUtil.sendPushReport(submit);
            throw new StrategyException(ExceptionEnum.BLACK_GLOBAL);
        }

        //4、通过检验日志
        log.info("【策略模块-平台级黑名单校验】平台级黑名单校验审核通过！");
    }
}
